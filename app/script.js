...
window.onload = function () {
  L.mapquest.key = '0aTGr9Al3b2iHwwRD6018q6Uc6zLP4Ij'; // 'map' refers to a <div> element with the ID map
  ...
  }).bindPopup('Welcome to St. Louis! <br> Find more info <a href="https://www.stlouis-mo.gov/">  here! </a>').addTo(map);
}; // search bar using JS
...
searchBar.addEventListener('keyup', e => {
  ...
  const filteredBreweries = brewerySpots.filter(data => {
    return data.name.toLowerCase().includes(searchString) || data.city.toLowerCase().includes(searchString) || data.state.toLowerCase().includes(searchString);
  });
...
breweryMarker.addEventListener('click'); //puts the breweries into a dropdown menu. 
